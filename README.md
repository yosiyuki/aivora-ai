...
- **Grounding の合否と本文からの除去はコードが行う。fail-closed。** 本文の全文・箇条書き・見出しが Claim で覆われていなければ、覆われていない部分を除去する。Fact は値が文中に含まれ、かつ文が Pack に無い数字を含まないときだけ根拠になる。本人の言葉（Experience）は分類に関わらず根拠になるが、参照だけでは足りず本文との一致が要る。根拠のない文は文単位で除去して `[[slot:キー]]` に置き換える。見出しが除去対象になれば版全体を failed にし、公開しない。
- **タイトルはコードが生成する**（主語の名前など）。モデルのタイトルは根拠を持たないため使わない。
...
**1 回答につき LLM 呼び出しは 1 回。** 分類・抽出・Archetype 仮説・スロット値・次の質問を 1 つの JSON Schema で返す。
往復回数がそのままコストになるため、分割しない。

判定（終了・Archetype 確定・充足）はすべてコードが行い、LLM は候補と確信度を出すだけ。
LLM の呼び出しが失敗しても回答は失われず、固定の質問でヒアリングは続く。
