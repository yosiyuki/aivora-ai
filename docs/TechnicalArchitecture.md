...
**全質問を自由記述とする。選択肢を提示しない。**

素人は専門用語を持たないだけで情報は持っており、
自分の言葉であれば詳しく語れる。
選択肢を出すと利用者はこちらの語彙に合わせ、その時点で情報が失われる。

自由記述の回答はそのまま Experiential Claim の原料となるため、
ヒアリングは設定行為であると同時に最初の Knowledge 収集を兼ねる。
...
ヒアリング回答も外部入力であり、Agent Isolation（§7）に例外を作らない。

### Archetype 判定

利用者に目的を選ばせず、発言から抽出する。
素人は目的を含んだ言い方をするため、動詞に目的が現れる。

判定できない場合のみ1問確認する。
Archetype は後から追加可能で URL も不変であるため（§23）、
初回判定の完璧さを求めない。

### 終了条件

minimum スロットが充足した時点で生成可能を提示する。
上限は10問程度とし、超えた場合は打ち切って記事生成へ進む。

残りのスロットは Verification Request として非同期に継続する。
