...
- `docs/TechnicalArchitecture.md` — 52 sections: components, agent isolation, policy/tool runtime, cost
  control, deployment shape
- `docs/DatabaseSchema.md` — 63 table definitions (§3–§65), indexes, pgvector/JSONB usage, phased rollout
...
Planned Phase 1 stack (`TechnicalArchitecture.md` §48): Rails, PostgreSQL, Solid Queue, optional pgvector,
LLM APIs, Docker. Application shape is a **modular monolith** (§49), not microservices — keep bounded
...
- **WordPress is an optional input source, not the source of truth and not a write target.** It has no
  special status among connectors (§23). There is no write-back, no field ownership negotiation, no
...
(`TechnicalArchitecture.md` §1, §52). Scores are computed in code; the LLM may explain but not decide
...
**Nothing is physically deleted** (`README.md` §23; `DatabaseSchema.md` §1, §67). PRUNE means unpublish +
...
**Human operation is required at initial setup only** (`README.md` §27, §30). There is no per-action
approval queue. The Review UI is an **after-the-fact observation surface**, not an approval gate
(`TechnicalArchitecture.md` §29), and it never offers content editing. Because individual review is gone,
...
(`README.md` §32; `TechnicalArchitecture.md` §37). Every LLM call is recorded in `llm_usage` — treat that
...
- AI may propose and infer policies, but **Editorial Policy prohibitions are product-fixed and can only be
  tightened, never loosened by the AI** (§29) — an LLM must not author its own constraints.
...
There is no launch/operation mode switch and **no `mode` state variable** (`README.md` §36;
`DatabaseSchema.md` §74). Each feature checks whether its own prerequisite data exists and enables itself:
...
they reject everything, which is why `README.md` §19 and §28 scope them to the operation phase.

## Implementation ordering

`DatabaseSchema.md` §73 lists the Phase 1 minimum tables. Two changes from v1.0 that affect priority:
...
tables (§74) have their schema defined but are enabled later.

Work is background-first, not realtime (`TechnicalArchitecture.md` §35). Model routing (§36): small for
